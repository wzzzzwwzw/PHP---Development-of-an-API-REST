...
class ApiResultsControllerTest extends BaseTestCase
{
    ...
    {
        $p_data = [
            Result::RESULT_ATTR => self::$faker->randomNumber(),
            Result::DATE_ATTR => self::$faker->date('Y-m-d H:i:s')
        ];
...
//    }
     ...
     * @depends testPostResultAction201Created
     */
    public function testResultStatus403Forbidden(string $method, string $uri, array $result): void
    {
        $userHeaders = $this->getTokenHeaders(
            self::$role_user[User::EMAIL_ATTR],
            self::$role_user[User::PASSWD_ATTR]
        );
        self::$client->request($method , $uri . '/' . $result['id'], [], [], $userHeaders);
        $response = self::$client->getResponse();
    ...
    {
        return [
            'getAction403'    => [ Request::METHOD_GET,    self::RUTA_API ],
            'putAction403'    => [ Request::METHOD_PUT,    self::RUTA_API ],
            'deleteAction403' => [ Request::METHOD_DELETE, self::RUTA_API ],
        ];
    }
}
